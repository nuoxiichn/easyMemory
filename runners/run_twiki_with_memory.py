...
from benchmarks.twiki_loader import ingest_twiki_sample, load_2wiki
from config_loader import init_from_config, load_config


def main() -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
    parser = argparse.ArgumentParser(description="Run 2Wiki ingest + RAG using configured memory/LLM.")
    ...
    config = load_config()
    datasets_cfg = config.get("datasets", {}).get("twiki", {}) if isinstance(config, dict) else {}
    ...
    table_name = db_tables.get("twiki")

    memory, llm_fn = init_from_config(config, table_name=table_name)

    ds = load_2wiki(split=split, path=path, fallback_to_dummy=True)
    if not ds:
        print("Failed to load 2Wiki sample.")
        return
    ...
    for idx, sample in enumerate(ds, start=1):
        ...
        result = memory.rag_answer(
            question=q,
            llm_fn=llm_fn,
            constraints={"dataset": "2wiki", "sample_id": sample_id},
            top_k=args.top_k,
        )
        line = {
            "index": idx,
